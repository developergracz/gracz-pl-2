		...
		if (oldPlayerId != null)
		{
			...
			if (oldPlayerId > 0)
			{
				...
				if (gameRoom.getSize().getUserCount() > 0)
				{
					...
					resObj.putInt("id", oldPlayerId);
					gameExt.send("stop", resObj, gameRoom.getUserList());
				}
			}
		}
		
		// if there's no more player, reset room
		if (gameRoom.getSize().getUserCount() == 0)
		{
			gameExt.standUp(1); gameExt.standUp(2);
			gameExt.trace("No more players, clean up.");
		}
	}
}
