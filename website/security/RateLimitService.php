...
final class RateLimitService
{
    ...
    public function consume($bucket, $identifier, $limit, $windowSeconds)
    {
        ...
        if ($this->pdo) {
            try {
                return $this->consumeDatabase($bucket, $key, $limit, $windowSeconds);
            } catch (Exception $e) {
                // Fall through to local limiter during a migration/temporary DB problem.
            }
        }
    ...
    {
        $table = $this->prefix.'_security_rate_limits';
        $now = time();
        $window = floor($now / $windowSeconds) * $windowSeconds;
        $windowEnd = $window + $windowSeconds;

        $sql = "INSERT INTO {$table} (bucket, identity_hash, window_start, hits, updated_at)
                VALUES (:bucket, :identity_hash, FROM_UNIXTIME(:window_start), 1, NOW())
                ON DUPLICATE KEY UPDATE hits = hits + 1, updated_at = NOW()";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute(array(':bucket' => $bucket, ':identity_hash' => $key, ':window_start' => $window));
        ...
        return array('allowed' => $hits <= $limit, 'remaining' => max(0, $limit - $hits), 'retry_after' => max(1, $windowEnd - $now));
    }

    private function consumeFile($bucket, $key, $limit, $windowSeconds)
    {
        $dir = sys_get_temp_dir().DIRECTORY_SEPARATOR.'gracz-rate-limit';
        if (!is_dir($dir)) {
            @mkdir($dir, 0700, true);
        }
        $window = floor(time() / $windowSeconds) * $windowSeconds;
        $file = $dir.DIRECTORY_SEPARATOR.hash('sha256', $bucket.'|'.$key.'|'.$window).'.json';
        $fp = @fopen($file, 'c+');
        if (!$fp) {
            // Fail closed for sensitive endpoints if limiter storage is unavailable.
            return array('allowed' => false, 'remaining' => 0, 'retry_after' => $windowSeconds);
        }
        flock($fp, LOCK_EX);
        $raw = stream_get_contents($fp);
        $state = $raw ? json_decode($raw, true) : array();
        $hits = isset($state['hits']) ? (int) $state['hits'] + 1 : 1;
        ftruncate($fp, 0);
        rewind($fp);
        fwrite($fp, json_encode(array('hits' => $hits, 'window' => $window)));
        fflush($fp);
        flock($fp, LOCK_UN);
        fclose($fp);
        return array('allowed' => $hits <= $limit, 'remaining' => max(0, $limit - $hits), 'retry_after' => max(1, ($window + $windowSeconds) - time()));
    }
    ...
    public function loginDelaySeconds($login, $ip)
    {
        ...
        $attempt = max(1, 1001 - (int) $result['remaining']);
    ...
    public function resetLoginFailures($login, $ip)
    {
        // Database implementation intentionally uses time windows rather than mutable counters.
        // A successful login is logged by AuditService; new successful auth does not need to erase evidence.
        return true;
    }
}
