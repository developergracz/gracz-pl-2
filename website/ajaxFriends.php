...
switch ($action)
{
  case 'add':
    try{
      AddToFriendList($idUser, $token);
      echo(json_encode(array('state' => 'added', 'id_user_friend' => $idUser)));
    }catch (Exception $e)
    {
  ...
  case 'remove':
    try{
      DeleteFromFriendList($idUser, $token);
      echo(json_encode(array('state' => 'removed', 'id_user_friend' => $idUser)));
    }catch (Exception $e)
    {
      echo(json_encode(array('state' => 'error', 'message' => $e->getMessage())));
    }
