...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
  ...
  header('Allow: POST');
  echo json_encode(array('state' => 'error', 'message' => 'Method not allowed.'));
...
switch ($action)
{
  case 'report':
  {
    try{
      ReportBug(
        ...
        isset($_POST['request_data']) ? $_POST['request_data'] : '',
  ...
  case 'delete':
  {
    try{
      ...
      echo(json_encode(array('state' => 'deleted')));
    }catch (Exception $e)
    {
      http_response_code(400);
      echo(json_encode(array('state' => 'error', 'message' => $e->getMessage())));
  ...
  {
    http_response_code(400);
    echo(json_encode(array('state' => 'error', 'message' => 'Unsupported action.')));
    break;
  }
}
