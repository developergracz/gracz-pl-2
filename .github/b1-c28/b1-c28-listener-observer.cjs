...
const outputPath = path.join(reportDir, `${runId}-b1-c28-listener-observer.json`);
...
try {
  ...
  console.error("B1-C28 listener observer failed to instrument pg:", error);
}
...
process.on("SIGUSR2", () => {
  ...
  try {
    ...
    console.error("B1-C28 listener observer could not establish steady window:", error);
  }
});
