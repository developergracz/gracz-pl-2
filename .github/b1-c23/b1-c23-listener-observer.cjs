...
const outputPath = path.join(reportDir, `${runId}-b1-c23-listener-observer.json`);
...
try {
  ...
  pg.Pool.prototype.connect = function b1c23ObservePoolConnect(callback) {
  ...
  pg.Client.prototype.query = function b1c23ObserveClientQuery(...args) {
  ...
  console.error("B1-C23 listener observer failed to instrument pg:", error);
}
...
process.on("SIGUSR2", () => {
  ...
  try {
    ...
    console.error("B1-C23 listener observer could not establish steady window:", error);
  }
});
