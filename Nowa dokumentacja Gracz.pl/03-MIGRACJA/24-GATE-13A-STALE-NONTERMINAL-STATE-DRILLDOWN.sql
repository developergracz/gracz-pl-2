-- ETAP 3 — Gate 13A: stale / nonterminal state drilldown
-- Data przygotowania: 29.08.2026
-- Tryb: READ ONLY / privacy-safe
--
-- Cel:
-- 1) sklasyfikować niekońcowe snapshoty Warcabów,
-- 2) sklasyfikować niekońcowe gry Tysiąc,
-- 3) sklasyfikować oczekujące relacje znajomych,
-- względem zatwierdzonego w Gate 12 zestawu LEGACY-IDENTITY / TEST.
--
-- Collector NIE wypisuje game_id, relation_id, user_id, nazw, e-maili,
-- tokenów, hashy ani żadnej treści prywatnej. Zwraca wyłącznie agregaty.
...
-- Zatwierdzony Gate 12 quarantine set (6/6):
-- gamerpl, gamerde, gracz.pl, gamerpolska, gamer, gracz.

-- A. Warcaby — klasyfikacja participant set + ślady faktycznego użycia snapshotu.
...
base AS (
  SELECT
    ...
    created_at,
    updated_at,
    j #>> '{game,status}' AS game_status,
    (SELECT COUNT(*)::int
       FROM jsonb_array_elements(COALESCE(j->'events','[]'::jsonb)) e
      WHERE e->>'type'='move.accepted') AS move_count
  FROM parsed
),
classified AS (
  SELECT
  ...
  CROSS JOIN LATERAL (
    VALUES
      (lower(b.j #>> '{players,white,id}')),
      (lower(b.j #>> '{players,black,id}'))
  ) p(user_id)
...
SELECT 'A_CHECKERS_CLASSIFICATION' AS section, metric, value
FROM (
...
-- B. Tysiąc — participant classification, revision i age.
WITH quarantine(user_id) AS (
  VALUES ('gamerpl'),('gamerde'),('gracz.pl'),('gamerpolska'),('gamer'),('gracz')
),
classified AS (
...
SELECT 'B_THOUSAND_CLASSIFICATION' AS section, metric, value
FROM (
...
-- C. Oczekujące relacje znajomych — tylko klasy agregatów.
WITH quarantine(user_id) AS (
  VALUES ('gamerpl'),('gamerde'),('gracz.pl'),('gamerpolska'),('gamer'),('gracz')
),
classified AS (
  SELECT
    ...
    f.created_at,
    f.updated_at,
    CASE WHEN qr.user_id IS NOT NULL THEN 'Q'
         WHEN ar.user_id IS NOT NULL THEN 'C'
         ELSE 'U' END AS requester_class,
    CASE WHEN qa.user_id IS NOT NULL THEN 'Q'
         WHEN aa.user_id IS NOT NULL THEN 'C'
         ELSE 'U' END AS addressee_class
...
SELECT 'C_FRIENDSHIP_PENDING' AS section, metric, value
FROM (
...
-- D. Reconciliation: source counts tylko jako liczby.
SELECT 'D_RECONCILIATION' AS section, metric, value
FROM (
