...
export class NewsletterService {
  ...
  async initialize() {
    if (!this.pool) return;
    await this.pool.query(`SELECT id,email,email_normalized,preferred_nick,preferred_nick_normalized,consent_version,consented_at,status,confirmation_token_hash,confirmation_expires_at,confirmation_sent_at,confirmed_at,position_token_hash,unsubscribe_token_hash,unsubscribed_at,created_at,updated_at FROM gracz_newsletter_subscribers LIMIT 0`);
  }
