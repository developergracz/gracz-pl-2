...
export class NewsletterService{
  ...
  async subscribe({email,consent,legal,preferredNick}){
    ...
    if(nick.normalized){const availability=await this.nicknameAvailable(nick.value);if(!availability.available){if(this.pool){const own=await this.pool.query(`SELECT 1 FROM gracz_newsletter_subscribers WHERE LOWER(TRIM(email))=$1 AND preferred_nick_normalized=$2 LIMIT 1`,[normalized,nick.normalized]);if(own.rowCount===0)throw Object.assign(new Error('Ten nick jest już zarezerwowany. Wybierz inny.'),{code:'NICK_TAKEN'});}else{const own=this.memory.get(normalized);if(own?.preferredNickNormalized!==nick.normalized)throw Object.assign(new Error('Ten nick jest już zarezerwowany. Wybierz inny.'),{code:'NICK_TAKEN'});}}}
    if(this.pool){await this.ready;try{
      await this.pool.query(`INSERT INTO gracz_newsletter_subscribers(email,email_normalized,preferred_nick,preferred_nick_normalized,consent_version,status,consented_at,unsubscribed_at,updated_at) VALUES($1,$1,$2,$3,'launch-v2','subscribed',NOW(),NULL,NOW()) ON CONFLICT(email) DO UPDATE SET email_normalized=EXCLUDED.email_normalized,preferred_nick=EXCLUDED.preferred_nick,preferred_nick_normalized=EXCLUDED.preferred_nick_normalized,status='subscribed',consent_version='launch-v2',consented_at=NOW(),unsubscribed_at=NULL,updated_at=NOW()`,[normalized,nick.value,nick.normalized]);
    }catch(error){if(error?.code==='23505')throw Object.assign(new Error('Ten nick jest już zarezerwowany. Wybierz inny.'),{code:'NICK_TAKEN'});throw error;}}
  ...
  async unsubscribe(email){const normalized=this.normalize(email);if(this.pool){await this.ready;await this.pool.query(`UPDATE gracz_newsletter_subscribers SET status='unsubscribed',unsubscribed_at=NOW(),updated_at=NOW() WHERE LOWER(TRIM(email))=$1`,[normalized]);}else this.memory.delete(normalized);return {ok:true};}
  async close(){if(this.pool)await this.pool.end();}
}

async function verifyTurnstile(secretKey,token){if(!secretKey)return true;if(!token)throw Object.assign(new Error('Nie udało się potwierdzić zabezpieczenia formularza. Spróbuj ponownie.'),{code:'TURNSTILE_REQUIRED'});const payload=new URLSearchParams({secret:secretKey,response:String(token)});let verification;try{const result=await fetch('https://challenges.cloudflare.com/turnstile/v0/siteverify',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body:payload,signal:AbortSignal.timeout(8000)});if(!result.ok)throw new Error(`Turnstile Siteverify HTTP ${result.status}`);verification=await result.json();}catch(error){throw Object.assign(new Error('Weryfikacja zabezpieczenia jest chwilowo niedostępna. Spróbuj ponownie za moment.'),{code:'TURNSTILE_UNAVAILABLE',cause:error});}if(verification?.success!==true)throw Object.assign(new Error('Weryfikacja bezpieczeństwa nie powiodła się. Spróbuj ponownie.'),{code:'TURNSTILE_FAILED'});return true;}
...
export function createNewsletterHandler(service,{siteKey=process.env.TURNSTILE_SITE_KEY||'',secretKey=process.env.TURNSTILE_SECRET_KEY||''}={}){const turnstileConfigured=Boolean(siteKey&&secretKey);return async(request,response)=>{const url=new URL(request.url,'http://localhost');const hostname=requestHostname(request);const turnstileEnabled=turnstileConfigured&&isProductionNewsletterHost(hostname);if(request.method==='GET'&&url.pathname==='/newsletter/challenge-config')return sendJson(response,200,{enabled:turnstileEnabled,provider:turnstileEnabled?'turnstile':null,siteKey:turnstileEnabled?siteKey:null});if(request.method==='GET'&&url.pathname==='/newsletter/nick-availability'){try{return sendJson(response,200,await service.nicknameAvailable(url.searchParams.get('nick')||''));}catch(error){return sendJson(response,400,{error:{code:error.code||'INVALID_NICK',message:error.message||'Nie udało się sprawdzić nicku.'}})}}if(request.method!=='POST'||url.pathname!=='/newsletter/subscribe')return false;let raw='';for await(const chunk of request){raw+=chunk;if(raw.length>10_000)throw Object.assign(new Error('Żądanie jest zbyt duże.'),{code:'REQUEST_TOO_LARGE'})}let body;try{body=JSON.parse(raw||'{}')}catch{body={}}try{if(turnstileEnabled)await verifyTurnstile(secretKey,body.challengeToken);return sendJson(response,201,await service.subscribe(body));}catch(error){const securityError=String(error.code||'').startsWith('TURNSTILE_');return sendJson(response,securityError?403:400,{error:{code:error.code||'NEWSLETTER_ERROR',message:error.message||'Nie udało się zapisać.'}})}};}
