...
export class GlobalChatService {
  ...
  async init() {
    await this.pool.query(`SELECT topic_id,owner_id,owner_name,title,description,category,created_at,closed FROM gracz_chat_topics LIMIT 0`);
    await this.pool.query(`SELECT message_id,user_id,display_name,body,reply_to,topic_id,reactions,created_at,edited_at,deleted FROM gracz_global_chat LIMIT 0`);
    await this.pool.query(`SELECT relation_id,requester_id,requester_name,addressee_id,addressee_name,status,created_at,updated_at FROM gracz_chat_friends LIMIT 0`);
    await this.pool.query(`SELECT report_id,message_id,reporter_id,reason,created_at FROM gracz_global_chat_reports LIMIT 0`);
  }
