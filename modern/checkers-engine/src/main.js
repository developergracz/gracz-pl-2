...
const rawGlobalChat=config.databaseUrl?new DistributedGlobalChatService({pool:store.pool,logger:secureLogger(audit)}):new GlobalChatService(null);await rawGlobalChat.ready;const globalChat=withChatModeration(rawGlobalChat,moderation);const globalChatHandler=createGlobalChatHandler({service:globalChat,auth,authSessions});
...
const lobby=new LobbyService({sessionStore:store,thousandService,gomokuService,pool:config.databaseUrl?store.pool:null});await lobby.ready;const platformLobbyHandler=createPlatformLobbyHttpHandler({lobby,auth,authSessions,trafficGuard:routedTrafficGuard,sharedTrafficGuard:distributedTraffic});
