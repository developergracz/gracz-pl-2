...
import { DistributedGlobalChatService } from "./distributed-global-chat.js";
...
import { GomokuRealtimeHub } from "./gomoku-realtime.js";
...
import { PostgresCommunityPool } from "./postgres-community-pool.js";
...
const community=config.databaseUrl?new PostgresCommunityPool(config.databaseUrl):null;
const distributedTraffic=config.databaseUrl?new PostgresDistributedTrafficGuard(config.databaseUrl):null;if(distributedTraffic?.ready)await distributedTraffic.ready;
const localTraffic=new TrafficGuard();
const {routedTrafficGuard,enforceRequest}=createProductionRateLimitComposition({localTrafficGuard:localTraffic,sharedTrafficGuard:distributedTraffic});
const checkersRealtime=config.databaseUrl?new PostgresRealtimeHub(config.databaseUrl,{logger:secureLogger(audit)}):null;if(checkersRealtime?.ready)await checkersRealtime.ready;
...
const securedAccounts=config.databaseUrl?new SecureAccountService(baseAccounts):baseAccounts;if(config.databaseUrl&&securedAccounts.ready)await securedAccounts.ready;
let accounts=withAccountModeration(securedAccounts,moderation);
const authSessions=config.databaseUrl?new PostgresAuthSessionStore(baseAccounts.pool):new MemoryAuthSessionStore();if(authSessions.ready)await authSessions.ready;
const messageAttachments=config.databaseUrl?new MessageAttachmentService(baseAccounts.pool,config.attachmentEncryptionKey):null;if(messageAttachments?.ready)await messageAttachments.ready;
...
const rawGlobalChat=config.databaseUrl?new DistributedGlobalChatService({pool:community.pool,logger:secureLogger(audit)}):new GlobalChatService(null);await rawGlobalChat.ready;const globalChat=withChatModeration(rawGlobalChat,moderation);const globalChatHandler=createGlobalChatHandler({service:globalChat,auth,authSessions});
...
const gomokuService=config.databaseUrl?new PostgresGomokuService(config.databaseUrl):new GomokuService();if(gomokuService.ready)await gomokuService.ready;const gomokuRealtime=new GomokuRealtimeHub({service:gomokuService,logger:secureLogger(audit)});await gomokuRealtime.ready;const gomokuHandler=createGomokuHttpHandler({service:gomokuService,auth,authSessions,realtime:gomokuRealtime});
const lobby=new LobbyService({sessionStore:store,thousandService,gomokuService,pool:config.databaseUrl?community.pool:null});await lobby.ready;const platformLobbyHandler=createPlatformLobbyHttpHandler({lobby,auth,authSessions,trafficGuard:routedTrafficGuard,sharedTrafficGuard:distributedTraffic});
const server=createGameHttpServer({store,accounts,authSessions,messageAttachments,auth,lobby,webRoot,logger:secureLogger(audit),realtime:checkersRealtime||undefined,trafficGuard:routedTrafficGuard,sharedTrafficGuard:distributedTraffic,sharedRequestGuardExternally:true});
...
server.on("request",async(request,response)=>{try{if(await handleHealthRequest(request,response,{store}))return;await enforceRequest(request);security.assertSameOrigin(request);if(await newsletterHandler(request,response))return;if(await adminSecurityHandler(request,response))return;if(await newsletterAdminHandler(request,response))return;if(await thousandHandler(request,response))return;if(await gomokuHandler(request,response))return;if(await platformLobbyHandler(request,response))return;if(await serveExtendedAsset(request,response))return;if(await globalChatHandler(request,response))return;if(await tournamentHandler(request,response))return;if(await rankingHandler(request,response))return;return baseRequestHandler(request,response);}catch(error){console.error("Application request error:",safeError(error));sendProductionRequestError(response,error);}});
...
let shuttingDown=false;async function shutdown(signal){if(shuttingDown)return;shuttingDown=true;console.log(`Otrzymano ${signal}. Zamykanie serwera…`);server.close(async()=>{try{thousandRealtime.close();await thousandService.close();if(checkersRealtime)await checkersRealtime.close();if(distributedTraffic)await distributedTraffic.close();gomokuRealtime.close();if(typeof gomokuService.close==="function")await gomokuService.close();await globalChat.close();if(typeof lobby.close==="function")await lobby.close();if(community)await community.close();if(typeof store.close==="function")await store.close();if(typeof accounts.close==="function")await accounts.close();if(typeof authSessions.close==="function")await authSessions.close();if(messageAttachments&&typeof messageAttachments.close==="function")await messageAttachments.close();await tournaments.close();await rankings.close();await newsletterLifecycle.close();await newsletterAdmin.close();await newsletter.close();await moderation.close();await mfa.close();await rbac.close();await audit.close();process.exit(0);}catch(error){console.error("Błąd podczas zamykania aplikacji:",safeError(error));process.exit(1);}});}for(const signal of["SIGINT","SIGTERM"])process.once(signal,()=>void shutdown(signal));
