...
const server=createGameHttpServer({store,accounts,authSessions,messageAttachments,auth,lobby,webRoot,logger:secureLogger(audit),realtime:checkersRealtime||undefined,sharedTrafficGuard:distributedTraffic,sharedRequestGuardExternally:Boolean(distributedTraffic)});
...
server.on("request",async(request,response)=>{try{if(await handleHealthRequest(request,response,{store}))return;if(distributedTraffic)await distributedTraffic.assertAllowed(request);security.assertSameOrigin(request);if(await newsletterHandler(request,response))return;if(await adminSecurityHandler(request,response))return;if(await newsletterAdminHandler(request,response))return;if(await thousandHandler(request,response))return;if(await gomokuHandler(request,response))return;if(await platformLobbyHandler(request,response))return;if(await serveExtendedAsset(request,response))return;if(await globalChatHandler(request,response))return;if(await tournamentHandler(request,response))return;if(await rankingHandler(request,response))return;return baseRequestHandler(request,response);}catch(error){console.error("Application request error:",safeError(error));if(!response.headersSent&&!response.writableEnded){if(error?.status===429&&Number.isFinite(error?.retryAfterSeconds))response.setHeader("Retry-After",String(Math.max(1,Math.ceil(error.retryAfterSeconds))));response.writeHead(error?.status||500,{"content-type":"application/json; charset=utf-8","cache-control":"no-store"});response.end(JSON.stringify({error:{code:error?.code||"APP_INTERNAL_ERROR",message:error?.status&&error.status<500?error.message:"Wewnętrzny błąd aplikacji."}}));}}});
