...
const accounts=config.databaseUrl?new SecureAccountService(baseAccounts,config.databaseUrl):baseAccounts;if(config.databaseUrl&&accounts.ready)await accounts.ready;
...
const globalChat=new GlobalChatService(config.databaseUrl||null);await globalChat.ready;const globalChatHandler=createGlobalChatHandler({service:globalChat,auth,authSessions});
...
const newsletter=new NewsletterService(config.databaseUrl||null,{mail});await newsletter.ready;const newsletterHandler=createNewsletterHandler(newsletter,{security});
...
const lobby=new LobbyService({sessionStore:store,thousandService});const platformLobbyHandler=createPlatformLobbyHttpHandler({lobby,auth,authSessions});
