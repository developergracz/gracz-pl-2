...
export class TournamentService {
  ...
  async init() {
    await this.pool.query(`SELECT tournament_id,owner_id,owner_name,title,description,game,format,status,visibility,max_players,rounds,time_control,rated,starts_at,current_round,created_at,finished_at FROM gracz_tournaments LIMIT 0`);
    await this.pool.query(`SELECT tournament_id,user_id,display_name,seed,points,wins,draws,losses,buchholz,status,joined_at FROM gracz_tournament_players LIMIT 0`);
    await this.pool.query(`SELECT match_id,tournament_id,round,board,white_id,white_name,black_id,black_name,result,status,reported_by,created_at,completed_at FROM gracz_tournament_matches LIMIT 0`);
  }
