(()=>{
  ...
  if(!form||!loginTab||!errorBox)return;
  ...
  function ensureMfaField(){
    ...
    label.id='privileged-mfa-field';
    label.textContent='Kod MFA administratora / moderatora';
    input=document.createElement('input');
    input.id='privileged-mfa-code';input.name='mfaCode';input.type='text';input.inputMode='numeric';input.autocomplete='one-time-code';input.maxLength=6;input.pattern='[0-9]{6}';input.placeholder='000000';input.required=true;
    ...
    const password=document.querySelector('#auth-password')?.closest('label');
    password?.after(label);
    return input;
  }

  form.addEventListener('submit',async event=>{
    if(!loginTab.classList.contains('active')||busy)return;
    event.preventDefault();event.stopImmediatePropagation();
    const data=Object.fromEntries(new FormData(form));
    if(document.querySelector('#privileged-mfa-code'))data.mfaCode=String(data.mfaCode||'').trim();
    busy=true;
    const submit=document.querySelector('#auth-submit');if(submit)submit.disabled=true;
    try{
      ...
      if(!response.ok){
        if(result.error?.code==='MFA_REQUIRED'||result.error?.code==='MFA_INVALID'){
          const input=ensureMfaField();errorBox.textContent=result.error?.message||'Wpisz kod MFA.';input.focus();return;
        }
