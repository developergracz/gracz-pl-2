(() => {
  ...
  function currentUser() {
    try { return JSON.parse(sessionStorage.getItem('gracz-session') || 'null')?.user || null; }
    catch { return null; }
  }
  ...
  function installGuestButton() {
    ...
    form.querySelector('#auth-submit')?.insertAdjacentElement('afterend', button);
    button.insertAdjacentElement('afterend', note);
    button.addEventListener('click', enterDemo);
  }
  ...
  async function enterDemo(event) {
    const button = event?.currentTarget || document.querySelector('#guest-thousand-demo') || document.querySelector('#open-thousand-demo');
    const note = document.querySelector('#guest-thousand-note');
    if (!button || button.dataset.busy === '1') return;
    const previousText = button.textContent;
    button.dataset.busy = '1';
    button.disabled = true;
    button.textContent = 'URUCHAMIAM DEMO…';
    if (note) note.textContent = 'Tworzę stół demonstracyjny Tysiąca.';

    try {
      const user = await ensureDemoUser();
      const suffix = String(user.userId).replace(/^guest-/, '').replace(/[^a-zA-Z0-9]/g, '').slice(0, 8) || Date.now().toString(36);
  ...
  function installCard() {
    ...
    card.id = 'thousand-module';
    card.className = 'game-card thousand-card';
    card.innerHTML = `
      <div class="card-top"><div><h2>TYSIĄC <span>ONLINE</span></h2><p>Klasyczna polska gra karciana dla 3 graczy.</p></div><span class="thousand-badge">3 GRACZY</span></div>
      ...
      <div class="game-card-foot thousand-card-foot">
        <div><strong id="thousand-room-count">0</strong><span>aktywnych stołów</span></div>
        <div class="thousand-card-actions">
          <button id="open-thousand-lobby" class="play-btn" type="button">GRAJ W TYSIĄCA →</button>
          <button id="open-thousand-demo" class="play-btn thousand-demo-btn" type="button">DEMO — ZOBACZ GRĘ</button>
        </div>
      </div>`;
    games.append(card);
    ...
    style.textContent = `
      ...
      .thousand-preview{height:240px;position:relative;display:grid;place-items:center;overflow:hidden;background:radial-gradient(circle at 50% 35%,#12372a 0,#091812 70%)}
      ...
      .thousand-card-foot{grid-template-columns:120px 1fr}.thousand-card-actions{display:grid!important;grid-template-columns:1fr 1fr;gap:8px}.thousand-card .play-btn{background:linear-gradient(180deg,#d9b94e,#a87d18);color:#15130d}.thousand-card .thousand-demo-btn{background:linear-gradient(180deg,#2d78d8,#1c56a6);color:#fff}
      @media(max-width:1100px){.thousand-card{width:100%;grid-column:1/-1}}
      @media(max-width:720px){.thousand-card{width:100%}.thousand-table-mini{width:90%;height:170px}.thousand-preview{height:215px}.thousand-hand-mini i{width:30px;height:43px}.thousand-card-foot{grid-template-columns:1fr}.thousand-card-actions{grid-template-columns:1fr}}
    ...
    document.querySelector('#open-thousand-demo')?.addEventListener('click', enterDemo);
  }

  function ensureModal() {
    if (modal) return modal;
    modal = document.createElement('div');
    modal.className = 'thousand-modal';
    modal.hidden = true;
  ...
  async function joinRoom(room) {
    try { await api(`/lobby/rooms/${encodeURIComponent(room.roomId)}/join`, { method: 'POST' }); await refresh(); }
    catch (error) { showStatus(error.message, true); }
  }
  ...
  async function invite(player) {
    ...
    try {
      ...
      showStatus(`Zaproszenie wysłane do: ${player.displayName}`);
    } catch (error) { showStatus(error.message, true); }
  }

  function showStatus(text, error = false) {
    const node = document.querySelector('#thousand-status');
    if (node) { node.textContent = text; node.style.borderColor = error ? '#b94b4b' : '#c9c0a6'; }
  }

  function renderCardState() {
    const counter = document.querySelector('#thousand-room-count');
    if (counter) counter.textContent = String(state.rooms.filter(room => room.gameType === GAME_TYPE).length);
    const button = document.querySelector('#open-thousand-lobby');
    if (button) {
      const room = ownRoom();
      button.textContent = room ? `STÓŁ ${room.filledSeats}/${room.maxPlayers} — ZAPROŚ →` : 'GRAJ W TYSIĄCA →';
    }
  }
  ...
  function renderModal() {
    ...
    const players = modal.querySelector('#thousand-players');
    ...
    const rooms = modal.querySelector('#thousand-rooms');
  ...
  async function answer(invitation, accept, box) {
    try {
      const result = await api(`/lobby/invitations/${encodeURIComponent(invitation.invitationId)}/respond`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ accept }) });
      box.remove();
      if (result.accepted && result.room?.status === 'playing' && result.room.gameId) {
        sessionStorage.setItem('thousand-entered-game', result.room.gameId);
        location.href = `/thousand.html?game=${encodeURIComponent(result.room.gameId)}`;
        return;
      }
      await refresh();
